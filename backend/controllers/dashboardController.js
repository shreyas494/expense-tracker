...
export async function getDashboardOverview(req,res){
    ...
    const borrowLendRecords = await borrowLendModel.find({ userId, status: { $ne: "settled" } }).lean();
    let totalBorrowed = 0;
    let totalLent = 0;
    for (const record of borrowLendRecords) {
      if (record.type === "borrow") {
        totalBorrowed += Number(record.remainingAmount || 0);
      } else {
        totalLent += Number(record.remainingAmount || 0);
      }
    }
    ...
    return res.status(200).json({
        ...
        console.log("GetDashboardOverview error:", err);
