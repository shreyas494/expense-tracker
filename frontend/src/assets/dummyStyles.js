...
export const dashboardStyles = {
  ...
  // Button styles
  addButton: "flex items-center gap-2 bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white px-5 py-3 rounded-xl transition-all shadow hover:shadow-md font-medium",
...
export const loginStyles = {
  pageContainer: "min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-4",
  cardContainer: "w-full max-w-md bg-white rounded-3xl shadow-xl border border-gray-100 p-8",
  header: "text-center mb-8",
  avatar: "w-16 h-16 bg-gradient-to-br from-teal-500 to-cyan-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-teal-500/20",
  headerTitle: "text-2xl font-bold text-gray-800",
  headerSubtitle: "text-sm text-gray-500 mt-2",
  formContainer: "space-y-6",
  errorContainer: "flex items-center gap-3 bg-red-50 text-red-600 p-4 rounded-xl text-sm border border-red-100 mb-6 animate-pulse",
  errorIcon: "flex-shrink-0 text-red-500",
  errorText: "font-medium",
  label: "block text-sm font-semibold text-gray-700 mb-2",
  inputContainer: "relative flex items-center",
  inputIcon: "absolute left-4 text-gray-400 pointer-events-none",
  input: "w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all duration-300 text-gray-800 font-medium placeholder-gray-400",
  paswordinput: "w-full pl-12 pr-12 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all duration-300 text-gray-800 font-medium placeholder-gray-400",
  passwordToggle: "absolute right-4 text-gray-400 hover:text-gray-600 focus:outline-none transition-colors",
  submitButton: "w-full py-3.5 bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white font-semibold rounded-xl shadow-lg shadow-teal-500/20 hover:shadow-xl hover:shadow-teal-500/30 active:scale-[0.98] transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 disabled:pointer-events-none",
  checkboxContainer: "flex items-center justify-between mb-6",
  checkboxLabel: "flex items-center gap-2.5 text-sm text-gray-600 cursor-pointer select-none",
  checkbox: "w-4 h-4 rounded text-teal-600 focus:ring-teal-500 border-gray-300 transition-colors",
  forgotPassword: "text-sm text-teal-600 hover:text-teal-700 font-medium transition-colors",
  footerText: "text-center text-sm text-gray-500 mt-8",
  footerLink: "text-teal-600 hover:text-teal-700 font-semibold transition-colors ml-1"
};
...
export const sidebarStyles = {
  // Layout and container styles
  sidebarContainer: {
    base: "hidden lg:flex flex-col pt-3 fixed top-[84px] bottom-0 z-30"
  },
...
export const styles = {
  ...
  statCards: {
    ...
    iconContainer: (color) => `bg-${color}-100 p-2 rounded-lg flex-shrink-0`,
  ...
  categories: {
    title: "text-lg md:text-xl lg:text-xl xl:text-xl font-bold text-gray-800 dark:text-gray-100 mb-6 flex items-center gap-3",
    titleIcon: "w-6 h-6 text-cyan-500",
    list: "space-y-4",
    categoryItem: "flex items-center md:text-lg lg:text-sm xl:text-lg justify-between",
    categoryIconContainer: "bg-teal-50 dark:bg-teal-950/50 p-2 rounded-lg text-teal-600 dark:text-teal-400 flex items-center justify-center w-8 h-8",
    categoryIcon: "w-4 h-4 text-teal-600 dark:text-teal-400",
    categoryName: "font-medium text-gray-700 dark:text-gray-300",
    categoryAmount: "font-semibold text-gray-800 dark:text-gray-200",
    summaryContainer: "mt-6 pt-6 border-t border-gray-100 dark:border-slate-800",
    summaryGrid: "grid grid-cols-2 gap-4",
    summaryIncomeCard: "bg-gradient-to-br from-teal-50 to-cyan-50 dark:from-slate-900/50 dark:to-teal-950/30 rounded-xl p-4 border border-transparent dark:border-teal-900/20",
    summaryExpenseCard: "bg-gradient-to-br from-orange-50 to-amber-50 dark:from-slate-900/50 dark:to-orange-950/30 rounded-xl p-4 border border-transparent dark:border-orange-900/20",
    summaryTitle: "text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider font-semibold",
    summaryValue: "text-base font-extrabold text-gray-800 dark:text-gray-100 mt-1",
  },

  // Color Helpers
  colors: {
    transaction: {
      text: (type) => (type === 'income' || type === 'borrow') ? 'text-teal-600' : 'text-orange-600',
      bg: (type) => (type === 'income' || type === 'borrow') ? 'bg-teal-100 text-teal-600' : 'bg-orange-100 text-orange-600',
    },
...
export const borrowLendStyles = {
  container: "min-h-screen p-4 md:p-6 space-y-6",
  ...
  tabButton: (isActive) => `px-6 py-3 font-semibold text-sm transition-all border-b-2 cursor-pointer ${
    isActive 
      ? "border-teal-500 text-teal-600" 
      : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
  }`,
  statsGrid: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6",
  statCard: "bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between gap-3",
  statTitle: "text-xs font-semibold text-gray-500 uppercase tracking-wider",
  statVal: "text-2xl font-bold text-gray-800 mt-1",
  statIcon: (color) => `p-2 rounded-xl bg-${color}-50 text-${color}-600 flex-shrink-0`,
  actionBar: "bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-wrap flex-col md:flex-row justify-between items-center gap-4 mb-6",
  statusBadge: (status) => {
    switch (status) {
      case 'settled':
        return "px-2.5 py-1 text-xs font-semibold rounded-full bg-green-50 text-green-700 border border-green-200";
      case 'partially_paid':
        return "px-2.5 py-1 text-xs font-semibold rounded-full bg-yellow-50 text-yellow-700 border border-yellow-200";
      default:
        return "px-2.5 py-1 text-xs font-semibold rounded-full bg-red-50 text-red-700 border border-red-200";
    }
  },
  overdueBadge: "px-2 py-0.5 text-[10px] font-bold rounded-lg bg-red-100 text-red-700 animate-pulse inline-block mt-1",
  dueSoonBadge: "px-2 py-0.5 text-[10px] font-bold rounded-lg bg-amber-100 text-amber-700 inline-block mt-1",
  cardList: "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5",
  itemCard: "bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden",
  cardHeader: "flex justify-between items-start mb-4",
  personName: "text-lg font-bold text-gray-800 truncate",
  amountLabel: "text-xs text-gray-500",
  amountVal: "text-xl font-extrabold text-gray-800",
  remainingVal: "text-sm font-semibold text-teal-600 mt-0.5",
  description: "text-sm text-gray-600 mt-2 line-clamp-2 min-h-[40px]",
  metaInfo: "flex items-center gap-1.5 text-xs text-gray-500 mt-4",
  repaymentItem: "flex justify-between items-center py-2 border-b border-gray-50 text-sm",
  repaymentNotes: "text-xs text-gray-400 block mt-0.5"
};

export const challengeStyles = {
  container: "min-h-screen p-4 md:p-6 space-y-6 bg-gray-50/50",
  ...
  card: "bg-white p-5 rounded-2xl border border-gray-150 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden",
  cardHeader: "flex justify-between items-start mb-3",
  cardTitle: "text-base font-bold text-gray-800 truncate",
  cardDesc: "text-xs text-gray-400 mt-1 line-clamp-2 min-h-[32px]",
  streakBadge: "flex items-center gap-1 text-xs font-bold text-orange-600 bg-orange-50 px-2.5 py-1 rounded-lg border border-orange-100",
  progressContainer: "space-y-1.5 my-4",
  progressBar: "w-full bg-gray-150 h-2 rounded-full overflow-hidden",
  progressFill: (color) => `h-full rounded-full transition-all duration-500 ${color}`,
  progressText: "text-[10px] font-bold text-gray-500 flex justify-between",
  badgeShowcase: "grid grid-cols-2 md:grid-cols-5 gap-4",
  badgeCard: (earned) => `bg-white border rounded-2xl p-4 text-center flex flex-col items-center justify-center transition-all ${
    earned 
      ? 'border-teal-200 bg-gradient-to-b from-white to-teal-50/30 text-teal-800 shadow-sm' 
      : 'border-gray-150 text-gray-400 grayscale opacity-60'
  }`,
  badgeName: "text-xs font-bold mt-2",
  badgeDesc: "text-[9px] text-gray-400 mt-0.5"
};